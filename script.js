const images = {
  ...
  '1.16': 'background-img/1.16.png',
  galactic: 'background-img/galac-client.png'
};
...
const games = {
  ...
  '1.16': {
    icon:'🧊', iconBg:'linear-gradient(135deg,#4fc3f7,#01579b)',
    version:'1.16.5', accent:'#4fc3f7', url:'https://itzpulsev2.github.io/1.16.5/u2/game.html',
  },
  galactic: {
    icon:'🌌', iconBg:'linear-gradient(135deg,#8a4fff,#2a0a5a)',
    version:'WASM Build', accent:'#8a4fff', url:'https://itzpulsev2.github.io/Galactic-Client/index%20(2)',
  }
};
...
function selectGame(gameKey, el) {
  ...
  const iconMap = {
    ...
    aero:   'icons/Mace.ico',
    '1.16': 'icons/1.16.ico',
    galactic: 'icons/Nether_Star.ico'
  };
  if (iconMap[gameKey]) {
  ...
  const names = {modern:'Modern Client',tuff:'Tuff Client',eb:'EB Client',resent:'Resent Client',death:'Death Client',aero:'Aero Client','1.16':'Eagler1.16',galactic:'Galactic Client'};
...
function renderDesc(gameKey) {
  const descs = {
    ...
    '1.16': {
      desc: `<strong>Eagler1.16</strong> is a browser-based Eaglercraft client built on the <strong>1.16 base</strong>, bringing a newer generation of Minecraft Java Edition to the web. It focuses on staying close to the modern vanilla experience while adding the quality-of-life tools browser players expect.<br><br>Running entirely in-browser, it requires no downloads or installs and works well across desktop and Chromebook hardware.`,
      features: [
        ["🧊","1.16 Base","Runs on the Nether Update base with its blocks, mobs and mechanics"],
        ["📊","HUD Overlays","FPS, CPS and armor status displays"],
        ["✨","Quality of Life","Toggle Sprint, Fullbright, and customizable UI options"],
        ["☁️","No Install","Runs fully in-browser — no downloads or installs needed"]
      ]
    },
    galactic: {
      desc: `The <strong>Galactic Client</strong> is a heavily modified Eaglercraft <strong>hack client</strong>, built with an extensive cheat and module system covering combat, movement, rendering and utility. It's designed to give players a competitive edge with a distinct space-themed UI.<br><br>Running entirely in-browser, it requires no downloads or installs and works across desktop and Chromebook hardware.`,
      features: [
        ["💀","Combat Mods","KillAura, Triggerbot, Velocity, RapidFire and more"],
        ["🌍","World Tools","Fullbright, NoFall, and other exploit modules"],
        ["🏃","Movement","Fly, Sprint, Blink, Step and other speed/movement cheats"],
        ["🌌","Space Theme","A distinct galactic-styled cheat UI"],
        ["📋","ArrayList","On-screen display of active modules"],
        ["☁️","No Install","Runs fully in-browser — no downloads or installs needed"]
      ]
    }
  };
  ...
  if (!d) { console.warn('No desc for key:', gameKey); return; }
  const descEl = document.getElementById("clientDescText");
  const featEl = document.getElementById("clientFeatures");
  console.log('renderDesc:', gameKey, d.desc.substring(0, 50));
  if (descEl) { descEl.innerHTML = ''; descEl.innerHTML = d.desc; }
...
async function loadVideoPlaylist() {
  try {
    const res = await fetch('videos/playlist.json');
    if (!res.ok) throw new Error('not found');
    const data = await res.json();
    videoFiles = data;
    renderVideoList();
  } catch(e) {
    console.warn('Could not load videos/playlist.json:', e);
    videoFiles = [];
    const el = document.getElementById('videoList');
    if (el) el.innerHTML = '<div class="video-list-empty">No videos found.<br>Add a <code>videos/playlist.json</code> file.</div>';
  }
}
...
function renderVideoList() {
  ...
  if (videoFiles.length === 0) {
    el.innerHTML = '<div class="video-list-empty">No videos in playlist.</div>';
    return;
  }
  el.innerHTML = videoFiles.map((v, i) => {
    const name = typeof v === 'string' ? v : v.name || v.file;
    const isPlaying = i === currentVideoIndex;
    return `<div class="video-item ${isPlaying ? 'playing' : ''}" onclick="playVideo(${i})">
      <div class="video-item-icon">${isPlaying ? '▶️' : '🎬'}</div>
      <div class="video-item-info">
        <div class="video-item-name">${name.replace(/\.mp4|\.webm|\.mkv|\.mov/gi,'')}</div>
        <div class="video-item-meta">${currentQuality} · Click to play</div>
      </div>
      ${isPlaying ? '<div class="video-item-playing-badge">▶ NOW PLAYING</div>' : ''}
    </div>`;
  }).join('');
}
...
function playVideo(index) {
  ...
  player.load();
  player.play().catch(() => {});
  if (titleEl) titleEl.textContent = '▶ ' + name.replace(/\.mp4|\.webm|\.mkv|\.mov/gi,'');
  renderVideoList();
  // Auto-play next when done
  player.onended = () => {
    if (currentVideoIndex < videoFiles.length - 1) playVideo(currentVideoIndex + 1);
  };
}
...
function setQuality(q) {
  ...
  const btn = document.getElementById('vq-' + (q === '4k' ? '4k' : '1080'));
  if (btn) btn.classList.add('active');
  // Update quality hint in list
  renderVideoList();
  // Note: actual quality depends on the video file itself
  // If you have separate 4K files, name them differently in playlist.json
}

// Load videos when Videos tab is opened
const _origSwitchTab = typeof switchTab !== 'undefined' ? switchTab : null;
function switchTabVideos(tabName, el) {
  switchTab(tabName, el);
  if (tabName === 'videos' && videoFiles.length === 0) loadVideoPlaylist();
}
