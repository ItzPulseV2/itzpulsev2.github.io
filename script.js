...
async function loadPlaylist() {
  try {
    ...
    console.warn('Could not load music/playlist.json');
    musicFiles = [];
  }
}
...
function selectGame(gameKey, el) {
  closeAllDropdowns();
  // If switching away from death, restore default dropdown content
...
async function autoStartMusic() {
  ...
  if (p !== undefined) {
    p.then(() => {
      ...
      // Browser blocked — reset and wait for first interaction
      musicOn = false;
